...
**- Human Driven for Smart Cars EV with remote communication to the Evox AI CORE in the Cloud;**

**- Expert System Driven for Smart Cars EV with remote communication to the Evox AI CORE in the Cloud;**

**- Smart Car EV using 5G Chip Interactive or Satellite Chip Interactive or Wi-Fi network;**

**- Smart Car EV using 5G Chip Interactive and Satellite Chip Interactive and Wi-Fi network;**
...
### Expert System as space invaders connecting EVOX AI CORE - Initial logic application
...
First, let's find where everything is located:

```bash
# Find mpirun location
find /usr -name "mpirun" 2>/dev/null

# Find MPI libraries
find /usr -name "libmpi.so*" 2>/dev/null

# Check OpenMPI directory structure
ls -la /usr/lib64/openmpi/
ls -la /usr/lib64/openmpi/bin/
```

Based on your include path, OpenMPI is likely installed in:
- **Include directory**: `/usr/include/openmpi-x86_64`
- **Library directory**: `/usr/lib64/openmpi/lib` or `/usr/lib64/openmpi`
- **Binary directory**: `/usr/lib64/openmpi/bin`
...
```bash
# Check if mpirun is now accessible
which mpirun
mpirun --version

# Check compiler wrappers
which mpicc
mpicc --version
```

## 4. Configure Eclipse CDT with the correct paths

Now let's configure Eclipse with the exact paths we found:
...
### Alternative: Use MPI Compiler Wrappers
...
## 7. Compile and run from terminal (for testing)
...
## 8. If mpirun still not found

If `mpirun` is still not found after setting PATH, try:

```bash
# Find the exact location
sudo find / -name "mpirun" -type f 2>/dev/null

# If found at a different location, update PATH accordingly
export PATH=/path/to/mpirun/directory:$PATH
```

## 9. Create a system-wide configuration (optional)

```bash
# Add OpenMPI to system PATH
sudo echo 'export PATH=/usr/lib64/openmpi/bin:$PATH' >> /etc/profile.d/openmpi.sh
sudo echo 'export LD_LIBRARY_PATH=/usr/lib64/openmpi/lib:$LD_LIBRARY_PATH' >> /etc/profile.d/openmpi.sh
...
![Image](https://github.com/user-attachments/assets/539a5450-3853-4118-a3c8-82f388b15b8d)

---

![Image](https://github.com/user-attachments/assets/1f7bb78a-0e79-4c86-ae84-e04b6db5c183)

---

![Image](https://github.com/user-attachments/assets/f14239bf-4699-4ef6-ad7f-9e2211638226)

---

![Image](https://github.com/user-attachments/assets/689bb350-5a1e-4b1d-92f3-692b4d82ef43)

---

![Image](https://github.com/user-attachments/assets/adf90f36-ae14-439f-beea-2ad5e1e0b6db)
